...
static bool tmc_start_receive(USBTMCDriver *tmcp) {
    ...
    usbStartReceiveI(tmcp->config->usbp, tmcp->config->bulk_out, buf,
...
static size_t _write(void *ip, const uint8_t *bp, size_t n) {
    ...
    return obqWriteTimeout(&((USBTMCDriver *)ip)->obqueue, bp, n,
                           TIME_INFINITE);
}

static size_t _read(void *ip, uint8_t *bp, size_t n) {

    return ibqReadTimeout(&((USBTMCDriver *)ip)->ibqueue, bp, n, TIME_INFINITE);
}
...
static msg_t _ctl(void *ip, unsigned int operation, void *arg) {
    ...
    switch (operation) {
        ...
        default:
            ...
            /* The TMC driver does not have a LLD but the application can use
               this hook to implement extra controls by supplying this
               function.*/
            extern msg_t tmc_lld_control(USBTMCDriver * tmcp,
                                         unsigned int operation, void *arg);
...
static void txDevDepMsgIn(USBTMCDriver *tmcp, uint8_t btag, uint8_t *txbuf,
                          size_t txsize);

/**
 * @brief   Notification of filled buffer inserted into the output buffers
 * queue.
 *
 * @param[in] bqp       the buffers queue pointer.
 */
    ...
    if (!usbGetTransmitStatusI(tmcp->config->usbp, tmcp->config->bulk_in)) {
        if (tmcp->next_btag) {
            ...
            if (buf != NULL) {
                ...
                tmcp->next_btag = 0;
                // usbStartTransmitI(tmcp->config->usbp, tmcp->config->bulk_in,
                // buf, n);
            }
        }
    }
...
void tmcObjectInit(USBTMCDriver *tmcp) {
    ...
    tmcp->state = TMC_STOP;
    ibqObjectInit(&tmcp->ibqueue, true, tmcp->ib, USB_TMC_BUFFERS_SIZE,
                  USB_TMC_BUFFERS_NUMBER, ibnotify, tmcp);
    obqObjectInit(&tmcp->obqueue, true, tmcp->ob, USB_TMC_BUFFERS_SIZE,
                  USB_TMC_BUFFERS_NUMBER, obnotify, tmcp);
}
...
static void txDevDepMsgIn(USBTMCDriver *tmcp, uint8_t btag, uint8_t *txbuf,
    ...
    usbStartTransmitI(tmcp->config->usbp, tmcp->config->bulk_in, txbuf,
                      (txsize + 3) / 4 * 4 + 12);
}
