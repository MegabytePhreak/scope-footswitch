...
{
    [SRC_BTN] 		   	= {LINE_BTN,		 	FLAG_BUTTON|FLAG_LONG_PRESS,	{EVT_BTN_CLICK,  EVT_BTN_HOLD}},
    [SRC_FOOTSW_BTN_1] 	= {LINE_FOOTSW_BTN1, 	FLAG_BUTTON|FLAG_INVERT, 		{EVT_FOOTSW1_PRESS, 0}},
    [SRC_FOOTSW_BTN_2] 	= {LINE_FOOTSW_BTN2, 	FLAG_BUTTON|FLAG_INVERT, 		{EVT_FOOTSW2_PRESS, 0}},
...
static void pal_event_cb(void* arg) {
    ...
    if(src < (sizeof(evt_config)/sizeof(evt_config[0])))
    {
        ...
        if(cfg->flags & FLAG_INVERT) pin_state = !pin_state;
	            ...
	            if(state->state == STATE_IDLE)
	            {
	                ...
	                if(cfg->flags & FLAG_LONG_PRESS) //Detect long press
	                {
	                	if(!chVTIsArmedI(&state->vt))
  							chVTSetI(&state->vt, TIME_MS2I(LONGPRESS_MS), longpress_cb, 0);
	                } else if (cfg->events[0]){ // Generate immediate event for low latecy processing
...
{
	chSysLock();
    iqObjectInit(&event_queue, event_queue_data, sizeof(event_queue_data), NULL, NULL);
