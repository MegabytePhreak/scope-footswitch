...
static void start_dma(WS2812Config * cfg, uint32_t len) {
	...
	dmaStreamSetPeripheral(cfg->dma_stream, &(cfg->pwm_driver->tim->CCR[cfg->pwm_channel]));
	dmaStreamSetMode( cfg->dma_stream,
...
static uint32_t encode_pixels(WS2812Config* cfg, uint32_t count, WS2812Pixel pixels[])
{
	uint32_t i;
